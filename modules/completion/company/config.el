...
(use-package! company-box
  ...
  (defun +company-box--load-all-the-icons ()
  ...
  ;; Load `company-box's icons
  (+company-box--load-all-the-icons)

  ;; NOTE Fix #6668: reload the icons after server creates a new frame, this
  ;; fixes an issue when launching Emacs as a daemon which causes `company-box'
  ;; to not show icons.
  (when (daemonp)
    (add-hook 'server-after-make-frame-hook #'+company-box--load-all-the-icons))
